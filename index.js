  ...
  data.forEach((item) => {
    const div = `
      <div class="home-audio-item">
        ...
        <p>${item.price}</p>
        <div onclick=requestDelete("${item.key}","${item.price}","audio")><i class="far fa-trash-alt"></i></div>
      </div>
    `;
...
btnmobilephone.addEventListener("click", () => {
  window.location.href = "mobilephone.html";
});
...
btnfashion.addEventListener("click", () => {
